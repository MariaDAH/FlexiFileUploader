...
export const PaginatedTable = ({
  ...
  const handleNext = async () => {
    ...
    const res = await fetch(
      `/api/public/v1/pagination?page=${nextPage}&limit=${PAGE_SIZE}`,
      {
        method: "POST",
        body: JSON.stringify({ data: totalData }),
      },
  ...
  const handlePrev = async () => {
    ...
    const res = await fetch(
      `/api/public/v1/pagination?page=${prevPage}&limit=${PAGE_SIZE}`,
      {
        method: "POST",
        body: JSON.stringify({ data: totalData }),
      },
  ...
  return (
    <div>
      ...
      <div className="flex justify-center">
        <button onClick={handlePrev} disabled={page === 1} className="m-3">
          Previous
        </button>
